...
private:
  ...
  {
    double k = Wmax_ * 0.03 *std::exp(  - 1.0 * ( std::abs(dt) / tau_plus_)) * (1 + cos(2*(dt)/tau_plus_)) -lambda_ * Wmax_ * 0.03 * mu_plus_ * std::exp(  - 1.0 *      (std::abs (dt) / 195.6)) * (1 - cos(2*(dt)/195.6));
    //std::cout << lambda_ << "\t" << mu_plus_<< std::endl;
    return k; 
  }
  double
  ...
  const nest::CommonSynapseProperties& )
{
  ...
  while ( start != finish )
  {
    ...
    dtp_ = t_spike - (start ->t_ + dendritic_delay);
