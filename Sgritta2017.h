...
private:
  ...
  {
    double k = alpha_*sin(3.14159265358979323846*dt*lambda_)*std::exp(-1*std::abs(tau_plus_ *dt));
  ...
  const nest::CommonSynapseProperties& )
{
  ...
  while ( start != finish )
  {
   //std::cout << start->t_ << "\t"<< t_lastspike << "\t" << t_spike  << std::endl;
    //if(t_lastspike == 0){
    dtp_ = (start ->t_ + dendritic_delay) - t_spike;
