	...
	for riga in range(1, len(N[:, 0])):
		...
		if(prova < prova_max):
			...
			if(N[riga, 1] > prova * t_prova and N[riga, 1] != N[riga - 1, 1] ): # > prova * t_prova, quando si usera' un file con piu' prove avra' piu' senso questa divisione di > e == 
				...
				if(math.floor(N[riga, 1]/t_prova) > prova  and N[riga, 0] == N[riga - 1, 0]): #N[riga, 1] <= (prova_max - 1) * t_prova

					col = math.floor(N[riga, 1]/t_prova)
					Aux[row_aux, 0] = N[riga - count, 0]
					for i in range(0, int(col) - prova + 1):
		...
		if(prova < prova_max): #se all'utlima prova non spara
			...
			if(riga == len(N[:, 0]) - 1 and math.floor(N[riga, 1]/t_prova) <= prova_max - 2 ): #prove e rapporti N[riga, 1]/t_prova) hanno una differenza di uno, quindi acccetto nek ciclo 
													   #valori di rapporto fino a 8 perche' equivalgono alla prova 9, accettare valori maggiori 
				Aux[row_aux, 0] = N[riga, 0]						   #significherebbe avere prove pari a 10 per le quali c'e' gia' un ciclo if apposito
				col = math.floor(N[riga, 1]/t_prova)
				for i in range(0, prova_max - prova + 1):
					Aux[row_aux, prova + i] = N[riga, 2]
				prova = prova_max + 1
...
print(E_Matrix[30:80, :])

''' PLOT DI VARIA NATURA E GENERE '''

#trials = np.arange(1, 11)
#i grafici mostrano un pezzo vuoto perche' i lplot apporrsima le x a 1600 ma abbiamo solo 1506 connessioni
plt.subplot(2, 5, 1)
plt.plot(E_Matrix[:, 1])
plt.title('Trial 1')
